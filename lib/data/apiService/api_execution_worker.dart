...
import '../../core/string_extension_function.dart';
...
class ApiExecutionWorker {
  ...
  final List<Map<String, dynamic>> _response = <Map<String, dynamic>>[];
  final Dio _dio;
  ...
  Future<Either<Failure, dynamic>> fetch(Map<String, dynamic> data) async {
    ...
    final String key = getRandomString(10);
    final Map<String, Object> responseMap = <String, Object>{
    ...
    int index = _response.indexWhere(
      (Map<String, dynamic> element) => element[ValueKeyResources.completerKey] == key,
    );
    _sendPort.send(data);
  ...
  void _handleConnectionAndResponse(dynamic message) {
    ...
    _response.last[ValueKeyResources.responseCompleter].complete(const Left(Failure("")));
  }
  ...
  static void _isolateEntry(List<dynamic> messages) {
    ...
    receivePort.listen((dynamic message) async {
      ...
      try {
        ...
        final Either<Failure, dynamic> response = await _executeAPI(dio: dio, reqParams: message);
  ...
  static Future<Either<Failure, dynamic>> _executeAPI({required Dio dio, required reqParams}) async {
    ...
    try {
      ...
      final Options headerOptions = Options(
      ...
      if (apiType == ApiType.GET) {
        response = queryParameters.isEmpty
            ? await dio.get(url, options: headerOptions)
            : await dio.get(
                url,
      ...
      if (e.response != null) {
        ...
        return Left(Failure(message, statusCode: e.response!.statusCode!));
      }
